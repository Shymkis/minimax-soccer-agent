...
class Game:
    ...
    def _run_round(self, speed):
        ...
        self.stats["red_start_x"] = state.players[0].x
        self.stats["red_start_y"] = state.players[0].y
        self.stats["blue_start_x"] = state.players[1].x
        self.stats["blue_start_y"] = state.players[1].y
        states = [state]
        i = -1
        ...
        while not state.is_terminal:
            ...
            if new_state in states:
                ...
                self.stats["avg_time"] = self.stats["total_time"] / self.stats["num_moves"]
                self.stats["red_end_x"] = new_state.players[0].x
                self.stats["red_end_y"] = new_state.players[0].y
                self.stats["blue_end_x"] = new_state.players[1].x
                self.stats["blue_end_y"] = new_state.players[1].y
                break
            states += [new_state]
            state = new_state
            ...
            self.stats["total_time"] += end - start
            self.stats["avg_time"] = self.stats["total_time"] / self.stats["num_moves"]
            self.stats["red_end_x"] = state.players[0].x
            self.stats["red_end_y"] = state.players[0].y
            self.stats["blue_end_x"] = state.players[1].x
            self.stats["blue_end_y"] = state.players[1].y
        ...
        if state and state.is_terminal:
            self.stats["winner"] = state.winner
        # with open("game_statistics.csv", "a", newline="") as f_object:
        #     dictwriter_object = DictWriter(f_object, fieldnames=self.stats.keys())
        #     dictwriter_object.writerow(self.stats)
        #     f_object.close()
